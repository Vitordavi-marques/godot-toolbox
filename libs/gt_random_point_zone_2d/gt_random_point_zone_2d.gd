...
func random_point() -> Vector2:
	...
	if rand_shape.shape is RectangleShape2D:
		return _random_point_rectangle(rand_shape)
	elif rand_shape.shape is CircleShape2D:
		return _random_point_circle(rand_shape)
	return Vector2.ZERO

func _random_point_rectangle(shape: CollisionShape2D) -> Vector2:
...
func _random_point_circle(shape: CollisionShape2D) -> Vector2:
